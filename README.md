# Решение для хакатона Siam ML Hack
...
Задача решалась в рамках хакатона Siam ML Hack(https://codenrock.com/contests/siam-ml-hack/). Заказчиком выступала компания «СИАМ»
...
- **Нестеров Никита** (БИВТ-23-1) — Машинное обучение (реализация алгоритма поиска монотонных участков и скрипта генерации признаков, писал классификаторы, ручная разметка). https://github.com/akayooo/akayooo
- **Калинин Алексей** (БИВТ-23-10) — Машинное обучение (реализация алгоритма поиска монотонных участков и скрипта генерации признаков, писал классификаторы, ручная разметка). https://github.com/ItamMigel
- **Савинов Святослав** (БИВТ-24-17) — Машинное обучение (реализация алгоритма поиска монотонных участков и скрипта генерации признаков, писал классификаторы, ручная разметка). https://github.com/cutroy
- **Глебов Максим** (БИВТ-24-17) — Машинное обучение (реализация алгоритма поиска монотонных участков и скрипта генерации признаков, писал классификаторы и ансамблирование, ручная разметка). https://github.com/treyder38
- **Колокольцев Ярослав** (БПИ-23-2) — Дизайнер (делал презентацию и подсказывал, как подавать решение). https://www.figma.com/design/LlTETGVIB37tq1V8D3j8mZ/%D0%9A%D0%BE%D0%BF%D0%B8%D0%B8-%D0%B3%D0%BB%D0%B0%D0%B2%D0%BD%D1%8B%D1%85-%D0%BF%D1%80%D0%BE%D0%B5%D0%BA%D1%82%D0%BE%D0%B2--%D0%AF%D1%80%D0%BE%D1%81%D0%BB%D0%B0%D0%B2-?node-id=0-1&t=Ba6GPxl4HJwQKnit-1
...
Этот проект представляет собой всесторонний и сбалансированный подход к анализу временных рядов и выгодно отличается от существующих аналогов:

- **Глубокий аналитический подход**  
  Мы не ограничивались «чёрным ящиком» машинного обучения, а провели глубокий разбор задачи, что позволило выявить скрытые закономерности.  
- **Качественная подготовка данных**  
  Основное внимание уделили обработке данных и генерации информативных признаков, благодаря чему модель демонстрирует высокую стабильность и точность.  
- **Стабильность и точность**  
  Разработанный алгоритм выявляет реальные тренды, а не подгоняется под тренировочные данные.  
- **Универсальность подхода**  
  Ручной поиск функций исключает пограничные ошибки и расширяет область применения решения.

### Возможные улучшения
1. **Адаптивный шумовой порог**  
   Введение динамического параметра шума позволит автоматически подстраивать чувствительность алгоритма под меняющуюся волатильность данных и корректно обрабатывать разные участки графика.  
2. **Новые статистические метрики**  
   Добавление показателей, таких как среднеквадратичное отклонение и дисперсия, повысит точность алгоритма в определении соответствия точек паттерну сегмента.  
3. **Глубокое исследование и валидация**  
   Тщательная экспериментальная проработка и валидация доработок укрепят разметку данных и повысят предсказательную способность модели.

> Внедрение этих улучшений позволит повысить точность, адаптивность и конкурентоспособность решения для анализа сложных временных рядов.
